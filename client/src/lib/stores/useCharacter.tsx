          ...
          cryptoAssets.forEach(crypto => {
            try {
              ...
              if (existingCryptoIndex >= 0) {
                // ALWAYS update crypto quantities, but we need to handle prices differently
                // for crypto market that trades 24/7
                
                // First, get the current price from the asset tracker or global prices
                const existingPrice = assetTracker.getAssetPrice(crypto.id);
                const globalPrice = assetTracker.globalCryptoPrices[crypto.id] || 0;
                const trackerPrice = existingPrice || globalPrice;
                
                // For crypto, we want to preserve price fluctuations that happen in the asset tracker
                // Only use character's price as a fallback if no valid price exists
                const finalPrice = (trackerPrice > 0) 
                  ? trackerPrice  // Use tracker's price if available
                  : crypto.currentPrice; // Fall back to character price if needed
                  
                console.log(`CRYPTO 24/7: Updating crypto in tracker: ${crypto.name} (${crypto.id}) with ${crypto.quantity} units at ${finalPrice}`);
                assetTracker.updateCrypto(crypto.id, crypto.quantity, finalPrice);
                
                // CRITICAL: Only update the global price during initial setup or if no price exists
                // Otherwise preserve price fluctuations generated by MarketPriceUpdater
                if (!assetTracker.globalCryptoPrices[crypto.id] || assetTracker.globalCryptoPrices[crypto.id] <= 0) {
                  console.log(`CRYPTO 24/7: Setting initial global crypto price for ${crypto.id} to ${finalPrice}`);
                  assetTracker.updateGlobalCryptoPrice(crypto.id, finalPrice);
                } else {
                  console.log(`CRYPTO 24/7: Preserving existing global crypto price for ${crypto.id}: ${assetTracker.globalCryptoPrices[crypto.id]}`);
                }
                
                cryptoUpdatedCount++;
              } else {
                // Add as a new crypto - this is a special case where we need to check if the global price exists
                
                // Check if there's already a global price for this crypto
                const globalPrice = assetTracker.globalCryptoPrices[crypto.id] || 0;
                
                // Determine which price to use - prefer existing global price if available
                const finalPrice = (globalPrice > 0) ? globalPrice : crypto.currentPrice;
                
                console.log(`CRYPTO 24/7: Adding new crypto to tracker: ${crypto.name} (${crypto.id}) with ${crypto.quantity} units at ${finalPrice}`);
                assetTracker.addCrypto({
                  id: crypto.id,
                  name: crypto.name,
                  amount: crypto.quantity,
                  purchasePrice: crypto.purchasePrice || finalPrice,
                  currentPrice: finalPrice
                });
                
                // Only set global price if it doesn't exist already
                if (!assetTracker.globalCryptoPrices[crypto.id] || assetTracker.globalCryptoPrices[crypto.id] <= 0) {
                  console.log(`CRYPTO 24/7: Setting initial global crypto price for ${crypto.id} to ${finalPrice}`);
                  assetTracker.updateGlobalCryptoPrice(crypto.id, finalPrice);
                } else {
                  console.log(`CRYPTO 24/7: Using existing global crypto price for ${crypto.id}: ${globalPrice}`);
                }
