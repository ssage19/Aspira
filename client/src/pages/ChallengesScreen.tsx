...
export default function ChallengesScreen() {
  ...
  // Handle keyboard shortcuts for time controls
  useEffect(() => {
  ...
  }, [toggleAutoAdvance, setTimeSpeed, autoAdvanceEnabled]);
  
  // Set up time advancement interval
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      const { advanceTime } = useTime.getState();
  ...
  // Effect to regularly check challenge progress
  useEffect(() => {
    // Check progress immediately
    checkChallengeProgress();
    
    // Then set up a regular interval to check progress 
  ...
  // Effect to refresh stats from stores
  useEffect(() => {
    const refreshInterval = setInterval(() => {
      // Just accessing these values will cause the component to re-render
      // when they change due to the way zustand works
      const { totalCash, totalNetWorth } = useAssetTracker.getState();
      const { job, income, netWorth } = useCharacter.getState();
      
      // This is a no-op, just forces a re-render if the values change
      if (totalCash && totalNetWorth && job && income && netWorth) {
        // No need to do anything, the component will re-render
      }
